# -*- coding: utf-8 -*-
"""
Created on Sun Sep 18 10:27:35 2022
...
class A:
    ...
    #Error block
    def exceptions(self,log_file ,*args):
        try:
            self.func(*args)
        except Exception as err:            
            self.Error_variable = 1
            
            now =datetime.now()
            time_str = now.strftime("%Y-%m-%d %H:%M:%S")
            log_file.write("Error time: " + time_str + "\n")              
            log_file.write( str(self.func.__name__) + ": " + str(err) + "\n")
            print(err)
    ...
    def __call__(self, *args):
        ...
        #Errors      
        self.Error_variable = 0
        A.exceptions(self,log_file, *args)
        
        if self.Error_variable == 0:
...
file = open('Output_task4.txt', 'w+')
log_file = open('log_task4.txt', 'w+')
...
with open("Output_task4.txt", "r") as f:
    contents = f.readlines()

contents.insert(0, "python task4.py\n")
...
contents.insert(2+unique_functions,"\n")

with open("Output_task4.txt", "w") as f:
